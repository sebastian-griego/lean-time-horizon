...
def main() -> int:
    ...
    protocol_deviations = read_csv(ROOT / "data" / "protocol_deviation_log.csv")
    ...
    protocol_deviation_status_counts = Counter(row.get("deviation_status", "unknown") for row in protocol_deviations)
    ...
    lines = [
        ...
        f"- `reports/analysis_decision_register.md`, `reports/evidence_strength_matrix.md`, `reports/protocol_deviation_log.md`, and `reports/statistical_analysis_plan.md` preregister analysis rules, grade evidence strength, record protocol deviations, set evidence thresholds, block overclaims, and define Wilson precision limits before broader model sweeps; decision statuses: `{compact_json(dict(sorted(analysis_decision_status_counts.items())))}`; evidence grades: `{compact_json(dict(sorted(evidence_strength_grade_counts.items())))}`; deviation statuses: `{compact_json(dict(sorted(protocol_deviation_status_counts.items())))}`.",
