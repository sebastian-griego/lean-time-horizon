...
def review_note(status: str, grade: str, hidden_wrong: int, public_wrong: int, negative: int) -> str:
    if status == "accepted_v0" and grade == "pins_not_exercised_by_wrongs":
        return "Accepted row needs a caveat: current wrong submissions fail before hidden pins run."
    ...
    if status == "accepted_v0" and negative == 0:
        return "Accepted row lacks negative hidden examples; review semantic pin strength before freeze."
...
def accepted_table(rows: list[dict[str, str]]) -> str:
    ...
    for row in accepted:
        lines.append(
            f"| `{row['task_id']}` | {row['pin_coverage_grade']} | {row['theorem_shape_checks']} | "
            f"{row['positive_examples']} | {row['negative_examples']} | {row['wrongs_failing_public_stage']} | "
