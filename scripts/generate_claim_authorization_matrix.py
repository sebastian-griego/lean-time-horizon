...
def build_rows() -> list[dict[str, str]]:
    ...
    rows.append(row(
        ...
        "Pair this with the limitations that broad provider sweeps, independent human timing, completed independent task reviews, and hosted QA are missing.",
        ...
        "Complete the planned model sweeps, collect independent timing and task-quality reviews, and add hosted QA/finding dispositions.",
        ["scaffold_result_comparison", "frontier_model_evidence", "independent_human_time_review", "independent_task_quality_review", "hosted_qa_env_linter"],
        [
    ...
    rows.append(row(
        ...
        f"{claim_evidence(claims, 'accepted_core_reviewed')}; {req_evidence(reqs, 'portfolio_accepted_count')}; {req_evidence(reqs, 'candidate_pruning_audit')}; {req_evidence(reqs, 'accepted_task_cards')}; {req_evidence(reqs, 'independent_task_review_packet')}; {req_evidence(reqs, 'independent_task_review_status_audit')}; {req_evidence(reqs, 'independent_task_quality_review')}; {req_evidence(reqs, 'diagnostic_coverage_audit')}; {req_evidence(reqs, 'construct_validity_matrix')}",
        "Completed independent Lean-human task-quality reviews plus more hard-reviewed T2/T3/T4 accepted tasks.",
        blocking_requirements(reqs, ["portfolio_accepted_count", "time_horizon_spread", "independent_task_quality_review"]).split(";") if blocking_requirements(reqs, ["portfolio_accepted_count", "time_horizon_spread", "independent_task_quality_review"]) else [],
        [
