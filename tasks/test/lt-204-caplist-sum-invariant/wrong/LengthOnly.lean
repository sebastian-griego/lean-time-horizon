...
theorem capOne_le_cap (cap x : Nat) :
    capOne cap x <= cap := by
  -- Same theorem signature, but this only works when x is already within cap.
  simp [capOne]
...
theorem capList_allLe (cap : Nat) (xs : List Nat) :
  ...
  -- Plausible but wrong: assumes identity projection preserves the bound.
  induction xs <;> simp [capList, allLe, *]
