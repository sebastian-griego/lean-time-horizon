...
| `regeneration_command_coverage` | commands | pass | commands=80; required=34; missing=[] | Command coverage proves the intended local gate is listed, not that it was run on a clean hosted environment. | Keep the README, manifest, and full validation command list in sync. |
| `artifact_hash_integrity` | artifact_hashes | pass | artifacts=168; checked_hashes=168; hash_mismatches=0; missing_recorded_paths=0; examples=[] | The manifest hashes generated local artifacts but intentionally avoids self-referential report hashes. | Regenerate the full local gate if any hash mismatch appears. |
...
| `git_snapshot_policy` | git_state | pass | dirty=True; status_entries=34; policy_note_present=True | A dirty generation-time snapshot is expected for committed report updates; this is not a clean-checkout proof. | For a release tag, run the full gate from a clean checkout or CI and record that evidence separately. |
