...
| `regeneration_command_coverage` | commands | pass | commands=98; required=43; missing=[] | Command coverage proves the intended local gate is listed, not that it was run on a clean hosted environment. | Keep the README, manifest, and full validation command list in sync. |
| `artifact_hash_integrity` | artifact_hashes | pass | artifacts=205; checked_hashes=205; hash_mismatches=0; missing_recorded_paths=0; examples=[] | The manifest hashes generated local artifacts but intentionally avoids self-referential report hashes. | Regenerate the full local gate if any hash mismatch appears. |
| `artifact_inventory_coverage` | artifact_hashes | pass | inventory_candidates=189; recorded_artifacts=205; allowed_unhashed=5; missing_inventory=0; examples=[] | The inventory check covers data CSVs, report markdown, and scripts. It intentionally excludes self-referential final reports, the validation-manifest audit output, and the progress log. | Add omitted evidence files to HASHED_ARTIFACTS or document them in ALLOWED_UNHASHED_ARTIFACTS. |
