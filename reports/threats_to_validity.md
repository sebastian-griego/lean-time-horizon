...
| `statistical_power_and_plots` | statistical_validity | high | block | statistical audit rows=8; blocked outputs=5 | Statistical reporting audit blocks unsupported pass-rate plots and requires raw n plus Wilson intervals. | Populate the planned sweep and accepted task count before generating scaffold, family, and bucket performance plots. | scaffold_effects;frontier_performance;family_level_performance |
| `failure_taxonomy_forecast` | internal_validity | medium | caution | accepted-core non-infra provider rows=1; expected failure modes are still mostly metadata forecasts | Failure labels and transcript workflow exist, but distributions are not interpreted until broad provider rows exist. | Label real model transcripts after the scaffold sweep and compare observed failures to intended diagnostic modes. | diagnostic_failure_distribution;scaffold_effects |
