...
| `requirement_status_counts` | report_counts | pass | `{"not_met": 3, "partial": 4, "supported": 66}` | `{"missing_report_needles": []}` | Requirement status counts are checked where the report summarizes supported/partial/not_met evidence. | `[]` | Regenerate requirement coverage and all report layers after requirement status changes. |
