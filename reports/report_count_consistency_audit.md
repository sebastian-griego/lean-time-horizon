...
| `requirement_status_counts` | report_counts | pass | `{"not_met": 3, "partial": 5, "supported": 64}` | `{"missing_report_needles": []}` | Requirement status counts are checked where the report summarizes supported/partial/not_met evidence. | `[]` | Regenerate requirement coverage and all report layers after requirement status changes. |
...
| `release_and_freeze_gate_counts` | gate_counts | pass | `{"freeze_readiness": {"block": 8, "caution": 1, "ready": 1}, "release_decision": {"block": 4, "caution": 3, "pass": 1}}` | `{"missing_report_needles": []}` | Release-decision and freeze-readiness status counts are checked wherever the report repeats them. | `[]` | Regenerate release/freeze reports and all report layers after gate-status changes. |
...
| `run_and_manifest_counts` | manifest_counts | pass | `{"local_qa_rows": 68, "model_rows": 3, "run_rows": 71}` | `{"manifest_run_summary": {"local_qa_row_count": 68, "local_qa_status_counts": {"expected_failure": 42, "passed": 26}, "model_failure_label_counts": {"infra_failure": 1, "none": 1, "proof_debugging": 1}, "model_sweep_row_count": 3, "row_count": 71}, "missing_report_needles": []}` | Run-result counts are checked against the validation manifest and the appendix manifest summary. | `[]` | Regenerate local QA rows, validation manifest, and report appendix after run-result changes. |
